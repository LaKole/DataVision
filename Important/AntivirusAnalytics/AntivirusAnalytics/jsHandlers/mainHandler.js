...
function startProgressTimer(x) {
    ...
    $('#progressTimer').progressTimer({
        ...
        //onFinish: function () { console.log('if its still running its too slow');}
    });
...
function filterHandler() {
    ...
    if (currentTab != '2d' && isPie === false) {
        ...
        if (column === 'version') {
            ...
            //remove version from row options
            filterPanel.find('#row option[value=version]').prop('disabled', true);
...
$('#create-chart-btn').on('click', getOptions);
...
$('#reset-alert').on('click', resetAlert);
...
function chart(querySet, method) {
    ...
    sendAjaxRequest(url, querySet, success);
...
function genericDraw(result, type, isRegenerated) {
    ...
    var options = {
        title: chartTitle,
        chartArea: { left: 75, top: 30, width: "70%", height: "85%" },
    ...
    if (type != 'table' && type != 'none') {
        ...
        if (!isRegenerated) {
            ...
            google.visualization.events.addListener(chart, 'ready', function () {
                $('#print-image').show().html('<a href="' + chart.getImageURI() + '">Printable Image</a>');
            });
        }
...
function signin() {
    ...
    sendAjaxRequest('/User/SignIn', userObject, success);
...
function register() {
    ...
    sendAjaxRequest('/User/Register', userObject, success);
...
function signout() {
    ...
    sendAjaxRequest('/User/SignOut', null, success);
...
//--------------------------------------------------------AJAX ----------------------------------------//
var ajaxPendingRequests = new Array();
function abortPendingRequests() {
    if (ajaxPendingRequests.length > 0) {
        ajaxPendingRequests.forEach(function (req) {
            //req.abort();
...
function sendAjaxRequest(url, obj, successFunction) {
...
//outside doc.ready so accessible to history partial view
function deleteImage($this) {
    ...
    sendAjaxRequest('/Home/DeleteHistory', { id: ci }, success);
...
function recreateImage($this) {
    ...
    sendAjaxRequest('/Visual/Regenerate', { id: ci }, success);
...
function loadHistory() {
    ...
    sendAjaxRequest('/Home/History', null, success);
}
